...
the teaching of a ``flipped'' course, where students would watch video % "would" is subjunctive, likely not appropriate
lectures online before class to prepare for classroom questions and
discussion. The design of Bottlenose also called for online submission % now is past tense
and grading of programming assignments, which turns out to be a useful
piece of functionality even for traditional courses.

Bottlenose is built using the Ruby on                                   % now is present tense
...
should help the application stay high quality and maintainable as it     % should - future subjunctive tense
...
more complicated assignments (submitted as a gzipped tarball) are       % "more complicated" is unclear, just say it
...
Bottlenose uses a sandbox mechanism to prevent student programs from    %weird run-on, maybe fix?
...
This sandbox mechanism isn't foolproof, but it has worked reasonably    % "foolproof" unclear
